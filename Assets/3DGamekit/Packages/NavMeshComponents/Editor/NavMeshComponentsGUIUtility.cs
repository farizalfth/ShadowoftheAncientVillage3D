...
namespace UnityEditor.AI
{
    public static class NavMeshComponentsGUIUtility
    {
        ...
        {
            var areaIndex = -1;
            // PERBAIKAN: Menggunakan GameObjectUtility untuk mengambil nama area
            ...
            if (EditorGUI.EndChangeCheck())
            {
                ...
                {
                    // PERBAIKAN: Cara membuka Project Settings Navigation di Unity modern
                    SettingsService.OpenProjectSettings("Project/Navigation Areas");
                }
            }
        ...
        public static void AgentTypePopup(string labelName, SerializedProperty agentTypeID)
        {
            ...
            if (EditorGUI.EndChangeCheck())
            {
                ...
                {
                    // PERBAIKAN: Membuka pengaturan Agen
                    SettingsService.OpenProjectSettings("Project/Navigation Agents");
                }
            }
        ...
        static void ToggleAgentMaskItem(SerializedProperty agentMask, int agentTypeID, bool value)
        {
            ...
            {
                if (agentMask.GetArrayElementAtIndex(j).intValue == agentTypeID)
                    idx = j;
            }
        ...
        {
            if (agentMask.arraySize == 0) return "None";
            if (IsAll(agentMask)) return "All";
            ...
            if (agentMask.arraySize <= 3)
            {
                ...
                for (var j = 0; j < agentMask.arraySize; j++)
                {
                    ...
                    if (string.IsNullOrEmpty(settingsName)) continue;
                    if (labelName.Length > 0) labelName += ", ";
